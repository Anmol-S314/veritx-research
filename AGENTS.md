...
- Vendored 2026-08-20: `floonoc/` — pulp-platform/FlooNoC + FlooGen (Apache-2.0 + SHL-0.51, TVLSI 2025), **ADOPTED as L3+L4 anchor** (c7fb CLOSED). Config→RTL generator + cycle-accurate NoC sim for AI accelerators. Venv: `floonoc/.venv` (uv, py3.12 — floogen 0.8.4; deps pydantic/networkx/mako/ruamel.yaml). Deps resolved via `floonoc/.tools/bender` v0.32.1 (`bender checkout` → `floonoc/.bender/git/checkouts`, pins in Bender.lock; no PATH change needed). RTL gen: `floonoc/.venv/bin/floogen rtl -c floogen/examples/nw_mesh_xy.yml` → `outputs_test/floo_nw_mesh_noc.sv`+`_pkg.sv` (4x4 mesh, SAM w/ 4 HBM + 16 cluster regions; HBM0 base = 0x000080000000). Sim: Verilator 5.032. DUT compile: `scripts/verilator/compile_dut.sh`; minimal tb compile+run: `scripts/verilator/compile_tb.sh` (`tb_minimal/tb_moe.sv` — AXI write cluster(0,0)→HBM(0) RESPONSE at cycle 19, b.resp OKAY). Gotchas: (1) upstream `floo_hbm_model`/`axi_test.sv` use virtual interfaces Verilator can't compile → hand-rolled behavioral `hbm_slave` in tb; (2) AXI structs from `AXI_TYPEDEF_ALL_CT` have NESTED channel fields (`req.aw.addr`, `req.w.data`, `req.aw_valid`/`req.w_valid` flat, `rsp.b_valid` + `rsp.b.resp` — NOT `rsp.b.valid`); (3) `VcCredit` generate branch dead for generated configs → suppress `-Wno-PINNOTFOUND`. Traffic: `floogen traffic` subcommand (uniform/hotspot/matmul... + `--traffic-cfg` traffic_flows) + tb `TRAFFIC_INJ_RATIO` plusarg. BookSim2 stays the L2 search engine; FlooNoC = L4 proof + L3 cross-check (full MoE dispatch on 64-node + BookSim2 cross-validation = ticket bfe1; energy vs Timeloop = ticket 2f0d).
