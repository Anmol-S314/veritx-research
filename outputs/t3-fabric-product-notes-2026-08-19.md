...
## 6c. Session addendum 2026-08-20 (2nd): static-vs-dynamic traffic — the F6 provisioning gap + FlooNoC ADOPTED

**FloNoC ADOPTED as L3+L4 anchor (ticket c7fb CLOSED, commit e1e8560).** All 4 criteria proven on this box: (1) DUT lints + compiles with Verilator 5.032 (bender 0.32.1, deps pinned in Bender.lock); (2) `floogen rtl -c floogen/examples/nw_mesh_xy.yml` generates 4x4-mesh RTL (SAM w/ 4 HBM channels + 16 cluster regions, HBM0 base 0x000080000000); (3) minimal tb (`floonoc/tb_minimal/tb_moe.sv`) simulates an AXI write cluster(0,0)→HBM(0) end-to-end — RESPONSE at cycle 19, b.resp OKAY (upstream floo_hbm_model/axi_test use virtual interfaces Verilator can't compile → hand-rolled behavioral hbm_slave instead); (4) 0.15 pJ/B/hop / 645 Gb/s/link claim (TVLSI 2025) accepted for adoption; energy cross-check deferred to ticket 2f0d. Follow-up: full MoE dispatch on 64-node FlooNoC + BookSim2 cross-validation = ticket bfe1. **BookSim2 stays the L2 search engine; FlooNoC is the L4 proof + L3 cross-check, NOT the DSE engine.**

**Static-vs-dynamic gap (the F6 provisioning hole):** our per-phase steady-state BookSim2 runs discard the burst envelope and inter-phase drain/backpressure. Primary sources confirm this matters for *sizing* (buffer/VC/saturation) not topology ranking at 64 nodes: ICCD'08 (self-similar traffic breaks Poisson buffer sizing), QuaLe (transient congestion ≠ steady state), SynFull/HPCA'16 (phase-structured replay is the cheap middle ground), AMD Multiphase NoC (per-phase provisioning industry concern). Full analysis + tool matrix + costed options (a) BookSim2 phase-sequenced replay, (b) FlooNoC job files, (c) gem5-Garnet TraceGen, (d) custom SynFull-style generator → **research-vendor/dyn-traffic/ + outputs/t3-dynamic-traffic-research-brief-2026-08-20.md (primary-source-verified, §8).** Recommendation: v1 = (a) cheap phase-sequenced BookSim2; product = (c) gem5-Garnet trace replay as dynamic validator with (b) FlooNoC as bit-true ground truth for final configs.
