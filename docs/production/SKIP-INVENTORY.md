...
| `test_astra_runtime.py:361` | `VERITX_ASTRA_REF_BIN` not set (two-binary 16/16 differential) | OPTIONAL_BACKEND | no (superseded by the independent timing oracle, R2) | backend | never; the differential is a shared-engine cross-check, not a numerical oracle |

`test_full_pipeline.py` no longer appears here: the six integration tests now
run against deterministic, tracked Chakra fixtures generated by
`veritx_dse.tools.gen_serving_chakra_fixtures` (R1.1), so the release gate no
longer skips on a missing developer-local trace directory.
...
1. `test_astra_runtime.py`'s two-binary differential is gated on an external
   reference binary. R2 replaced it as the required numerical evidence with an
   independent closed-form timing oracle (`test_astra_timing_oracle.py`), so
   the differential is no longer release-critical. The backend binaries
   (BookSim, ASTRA) are resolved from the repository build; a missing binary
   skip is release-critical, so the real backend gates cannot pass by
   skipping.
2. The six full-pipeline integration tests no longer skip: deterministic
   Chakra `.et` fixtures are generated from canonical text traces and
   committed under `tests/fixtures/serving_chakra/` with a digest manifest
   (`R1.1`). `test_serving_fixture_provenance.py` proves the committed bytes
