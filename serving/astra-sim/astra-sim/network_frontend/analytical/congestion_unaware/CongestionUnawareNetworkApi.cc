...
int CongestionUnawareNetworkApi::sim_send(void* const buffer,
    ...
    // compute send communication delay (in AstraSim format). A self-send
    // (dst == src) is a LOCAL copy — the collective's local-chunk reduction
    // — not a network transfer: zero latency, no topology call (the
    // analytical backend asserts src != dest).
    double send_delay = 0.0;
    if (dst != src) {
        const auto send_delay_ns = topology->send(src, dst, count);
        send_delay = static_cast<double>(send_delay_ns);
    }
