...
class EventQueue {
 public:
  ...
  void run_cycles(int cycles) {
    // Chunked stepping with interleaved drains + early exit. A blind N-cycle
    // chunk retires a packet at +50 but only pumps it at +N, quantizing every
    // collective step to N cycles (observed: 1M/step, 30M+ per allreduce,
    // multi-hour full traces at 100% CPU with zero output). Draining every 1K
    // and returning as soon as arrivals flow lets the event loop consume
