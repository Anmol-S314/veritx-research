...
void Sys::handleEvent(void* arg) {
    ...
    if (event == EventType::CallEvents) {
        // Process events for ALL systems at this tick (not just the one that scheduled)
        // This is needed because register_event only calls sim_schedule once per unique tick,
        // so other systems' events at the same tick would be missed.
        for (auto* sys : all_sys) {
            if (sys != nullptr) sys->call_events();
        }
        delete ehd;
