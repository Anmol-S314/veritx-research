...
class Workload : public Callable {
  public:
    ...
    void issue_broadcast(std::shared_ptr<Chakra::FeederV3::ETFeederNode> node);
  ...
  private:
    ...
    // VeritX native broadcast: root issues k-1 concurrent sends, every other
    // rank one recv; the group's DataSet completes when all arrive.
    static void broadcast_packet_complete(void* arg);
    void complete_broadcast(void* arg);
};

// VeritX native broadcast bookkeeping (node_id + pending packet count).
struct BroadcastData {
  Workload* workload;
  DataSet* fp;
  int remaining;
  uint64_t coll_comm_id;
};
