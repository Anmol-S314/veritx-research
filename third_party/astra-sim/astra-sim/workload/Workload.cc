...
void Workload::issue(shared_ptr<Chakra::FeederV3::ETFeederNode> node) {
    ...
    if (sys->replay_only) {
        ...
        if ((node->type() == ChakraNodeType::MEM_LOAD_NODE) ||
            ...
            issue_metadata(node);
        // Chakra v3 node types: handle gracefully
        } else if (node->type() == 8) {  // STORAGE_NODE
            issue_remote_mem(node);
        } else if (node->type() == 9) {  // DATA_OP_NODE
            if (!this->sys->roofline_enabled) {
                issue_replay(node);
            } else {
                issue_comp(node);
            ...
            logger->critical("Unknown node type {}", static_cast<uint64_t>(node->type()));
            exit(EXIT_FAILURE);
        }
    }
}
...
void Workload::issue_coll_comm(
    ...
    if (comm_type == ChakraCollectiveCommType::ALL_REDUCE) {
        ...
        // k-1 CONCURRENT sends in one handler so the NIC-side multicast fold
        // can collapse them into one fabric stream per flit; every other rank
        // issues one recv. The group's DataSet completes when all arrive.
        issue_broadcast(node);
    } else {
        throw std::runtime_error("Unsupported collective comm type");
    }
}

void Workload::issue_broadcast(
    shared_ptr<Chakra::FeederV3::ETFeederNode> node) {
    const uint64_t size = node->comm_size<uint64_t>();
    // root: the rank that broadcasts (default 0; set via the "bcast_root"
    // attribute so MoE dispatch sources can rotate across ranks)
    const uint32_t root = node->get_attr<uint32_t>("bcast_root", 0u);
    // tag must match between the root's sends and the receivers' recvs
    const uint32_t tag = static_cast<uint32_t>(node->id());
    ...
    bd->workload = this;
    bd->fp = fp;
    bd->coll_comm_id = fp->my_id;
    bd->remaining = 0;

    const int n = sys->logical_topologies["AllGather"]
                          ->get_num_of_nodes_in_dimension(0);
    if (sys->id == static_cast<int>(root)) {
        // root: k-1 concurrent sends in one handler (the multicast fold's
        // trigger condition)
        for (int dst = 0; dst < n; ++dst) {
            if (dst == static_cast<int>(root)) continue;
            bd->remaining++;
            sim_request req;
            req.srcRank = 0;
            req.dstRank = dst;
            req.reqType = UINT8;
            req.reqCount = size;
            sys->front_end_sim_send(0, Sys::dummy_data, size, UINT8, dst, tag,
                                    &req, Sys::FrontEndSendRecvType::NATIVE,
                                    &Workload::broadcast_packet_complete, bd);
        }
    } else {
        // receiver: one recv from the root
        bd->remaining = 1;
        sim_request req;
        req.srcRank = static_cast<int>(root);
        req.dstRank = sys->id;
        req.reqType = UINT8;
        req.reqCount = size;
        sys->front_end_sim_recv(0, Sys::dummy_data, size, UINT8,
                                static_cast<int>(root), tag, &req,
                                Sys::FrontEndSendRecvType::NATIVE,
                                &Workload::broadcast_packet_complete, bd);
    }

    if (bd->remaining == 0) complete_broadcast(bd);
}

void Workload::broadcast_packet_complete(void* arg) {
    BroadcastData* bd = static_cast<BroadcastData*>(arg);
    if (--bd->remaining > 0) return;
    bd->workload->complete_broadcast(bd);
}

void Workload::complete_broadcast(void* arg) {
    BroadcastData* bd = static_cast<BroadcastData*>(arg);
    // fires CollectiveCommunicationFinished with bd->coll_comm_id; the
    // completion path releases the node, resolves the DAG and deletes fp
    bd->fp->call(EventType::General, nullptr);
    delete bd;
}
