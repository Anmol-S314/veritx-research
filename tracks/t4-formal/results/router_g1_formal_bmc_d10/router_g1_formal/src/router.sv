...
module noc_router #(
  ...
  input  logic            clk,
  input  logic            rst_n,
  input  noc_pkg::link_f_t [0:4]        flit_in,
  input  noc_pkg::link_c_t [0:4]        credit_in,
  input  noc_pkg::link_c_t [0:4]        credit_in_early,
  output noc_pkg::link_f_t [0:4]        flit_out,
  output noc_pkg::link_c_t [0:4]        credit_out,
...
`endif
  ...
  function automatic logic [2:0] xy_dor(
    ...
    if (dx != x) xy_dor = (dx > x) ? PORT_E : PORT_W;
    else if (dy != y) xy_dor = (dy > y) ? PORT_N : PORT_S;
    else xy_dor = PORT_L;
  endfunction
