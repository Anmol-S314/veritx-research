"""Attachment tests — topology-bound hardware identity and interface authority.

Attachment identity is topology-bound hardware semantics only. DesignRevision
and NodeInventory are validation sources; MappingArtifact is a downstream
ResolvedFabric concern. The same hardware with a different mapping, or an
unrelated design change, must keep the same attachment_hash.
"""
...
def test_wrong_or_missing_type_tag_refused():
    _cr_a, _inv, _t, att = _derive(
        [Agent(kind=AgentKind.COMPUTE_TILE, count=4)])
    wrong = att.to_dict()
    wrong["type"] = "srota/TopologyArtifact"
    with pytest.raises(AttachmentError, match="type tag"):
        AgentAttachmentArtifact.from_dict(wrong)
    missing = att.to_dict()
    del missing["type"]
    with pytest.raises(AttachmentError, match="type tag"):
...
def test_interface_descriptor_rejects_empty_strings(kwargs):
    fields = dict(data_width_bits=256, address_width_bits=64, protocol="AXI",
                  clock_domain=None, power_domain=None)
...
def test_interface_descriptor_from_dict_rejects_empty_protocol():
    with pytest.raises(ValueError):
        AgentInterfaceDescriptor.from_dict({
            "data_width_bits": 256, "address_width_bits": 64,
            "protocol": "", "clock_domain": None, "power_domain": None})
