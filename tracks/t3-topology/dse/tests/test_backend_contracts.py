...
class TestConfigIdentity:
    def test_config_hash_is_domain_separated_sha256(self):
        ...
        a = _artifact()
        body = (("srota/BackendConfig/v2\0").encode()
                + canonical_bytes(a.identity_dict()))
        assert a.backend_config_hash() == hashlib.sha256(body).hexdigest()
...
class TestInputManifest:
    def test_hash_is_domain_separated(self):
        ...
        from veritx_dse.core.artifact import canonical_bytes
        m = _manifest()
        body = ("srota/BackendInputManifest/v1\0").encode() + canonical_bytes(
            m.identity_dict())
        assert m.backend_input_hash() == hashlib.sha256(body).hexdigest()
