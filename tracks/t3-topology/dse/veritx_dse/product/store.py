...
One directory per project. All writes are atomic (temp file + rename) and
serialized by a process lock, so a crash cannot leave a half-written
resource. No database is introduced: the durable form is JSON on disk.
