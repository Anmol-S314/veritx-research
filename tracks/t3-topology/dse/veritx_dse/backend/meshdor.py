...
def _hash_of(obj, name):
    ...
    this shim only normalizes the accessor (veritx-integrate adapter:
    the canonical spine stores hashes as attributes)."""
...
def lower_meshdor_standalone(
    ...
    t_hash, a_hash = _hash_of(topo, 'topology_hash'), _hash_of(att, 'attachment_hash')
    rra_hash, vc_hash = _hash_of(rra, 'resolved_route_hash'), _hash_of(vc, 'vc_assignment_hash')
    pf_hash, rb_hash = _hash_of(pf, 'packet_format_hash'), _hash_of(rb, 'router_behavior_hash')
    ad_hash = _hash_of(ad, 'address_decode_hash')
    ...
    bindings = (
        ...
        bind(SemanticDimension.PLANE_COMPOSITION, _hash_of(fabric, 'fabric_hash'),
    ...
    try:
        artifact = BackendConfigArtifact(
            ...
            fabric_hash=_hash_of(fabric, 'fabric_hash'),
...
def assert_canonical_meshdor_projection(
    ...
    if config.fabric_hash != _hash_of(bundle.fabric, 'fabric_hash'):
        raise BookSimLoweringError(
...
def _execute_prepared_meshdor(
    ...
    if config.fabric_hash != _hash_of(bundle.fabric, 'fabric_hash') or \
    ...
    return CertifiedBookSimEvidence(
        ...
        producer_source_dirty=producer.dirty,
        producer_source_dirty_digest=producer.dirty_digest,
