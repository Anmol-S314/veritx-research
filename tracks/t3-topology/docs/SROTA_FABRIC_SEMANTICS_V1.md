...
| 1.27 | B-FINAL.1 | Producer and evidence-reuse integrity. `verify_evidence_binding` (and the composed `verify_reusable_evidence`) now requires a pinned producer first, so unpinned/dirty/unknown-dirt producers can never reuse evidence; `assert_pinned_producer` uses the exact `source_pinned` invariant with distinct errors for missing revision, unknown dirt state, and dirt. The dirty digest is a content digest (`git diff --no-ext-diff --no-textconv --binary HEAD --`: staged and unstaged tracked changes, deletions, mode changes), so different edits to the same files produce different digests; untracked files remain excluded by policy. Persistence is content-identified by the external `EvidenceRef(path, sha256)`; `read_verified_evidence` refuses bytes that no longer match, and the composed reuse operation proves pinning, digest, parse, config, input and producer in order. The runner rehashes the binary immediately before spawn and refuses on any hash-to-exec change with zero spawns. |
| 1.28 | B-FINAL.2 | Certified execution authenticity. `run_qualified_booksim` no longer accepts an injected runner: it always spawns the identified binary through the authoritative supervised process runner, so `EXECUTED_*` evidence implies an actual execution. Unit tests use the explicitly test-only `_run_qualified_booksim_with_runner_for_test`, whose products carry `execution_transport=TEST_INJECTED` and are mechanically refused by the reuse API. The executed-route output must be fresh: a pre-existing `routing.dump` refuses before spawn, and a missing one after execution still refuses. The binary path is resolved strictly to one absolute path before hashing, and that same path feeds the initial digest, the pre-spawn recheck, argv and evidence. Digest-verified reuse is the only public reuse API (`verify_reusable_evidence`); the field-only binding helper is private and `read_evidence` is documented inspection-only. |
