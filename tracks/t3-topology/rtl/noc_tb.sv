...
module noc_tb;
  ...
  // per-node NIC control + counters
  logic [1:0]  gmode[N];
  logic [31:0] blen[N], r0[N], r1[N], sd[N];
  logic       tw[N];
  logic [9:0] ta[N];
  logic [63:0] td[N];
  logic [31:0] tck[N], injc_[N], ejc_[N];
  logic [31:0] lsum[N][4], lcnt[N][4];
  logic [63:0] dbg_trace0[N];
  logic [31:0] dbg_tptr[N];
  logic        dbg_pending[N];
...
`endif
        ...
        .clk(clk), .rst_n(rst_n),
        .inject(inj[y][x]), .inject_credit(injc[y][x]),
        .inject_credit_early(injce[y][x]),
        .eject(ej[y][x]), .eject_credit(ejc[y][x]),
...
`ifdef R1_MODE
    ...
    $display("R1 replay: %0d NICs, %0d cycles", N, RUN_CYCLES);
    for (int n = 0; n < N; n++) gmode[n] = 2'd2;
    for (int t = 0; t < RUN_CYCLES; t++) begin
      for (int n = 0; n < N; n++) begin
        if (ej[n / X_DIM][n % X_DIM].valid) begin
          $fwrite(fd, "%0d %0d %0d %0d %0d %0d\n",
                  tck[n] - replay_base,
                  ej[n / X_DIM][n % X_DIM].flit.cl,
                  ej[n / X_DIM][n % X_DIM].flit.src,
                  ej[n / X_DIM][n % X_DIM].flit.dst,
                  ej[n / X_DIM][n % X_DIM].flit.pid,
                  ej[n / X_DIM][n % X_DIM].flit.itime - replay_base);
        end
      end
      if (t % 500 == 0) begin
        longint tot;
        tot = 0;
        for (int n = 0; n < N; n++) tot += ejc_[n];
      ...
      for (int y = 0; y < Y_DIM; y++)
        for (int x = 0; x < X_DIM; x++) begin
          for (int p = 0; p < NUM_PORTS; p++) begin
            int o;
            o = rrecv[y][x][p] - rpop[y][x][p];
            $display("DBG3 R%0d,%0d port%0d occ=%0d recv=%0d pop=%0d",
                     x, y, p, o, rrecv[y][x][p], rpop[y][x][p]);
            for (int v = 0; v < VCS; v++)
              $display("DBG4 R%0d,%0d i%0d v%0d st=%0d op=%0d ov=%0d cf=%0d iu=%0d oc=%0d",
                       x, y, p, v, rdbg[y][x].st[p][v], rdbg[y][x].out_port[p][v],
                       rdbg[y][x].out_vc[p][v], rdbg[y][x].credit_free[p][v],
                       rdbg[y][x].in_use[p][v], rdbg[y][x].occ[p][v]);
          end
          $display("DBG5 R%0d,%0d cfE%0d cfW%0d cfN%0d cfS%0d cfL%0d iuE%0d iuW%0d iuN%0d iuS%0d iuL%0d",
                   x, y, rdbg[y][x].credit_free[0][0], rdbg[y][x].credit_free[1][0],
                   rdbg[y][x].credit_free[2][0], rdbg[y][x].credit_free[3][0],
                   rdbg[y][x].credit_free[4][0], rdbg[y][x].in_use[0][0],
                   rdbg[y][x].in_use[1][0], rdbg[y][x].in_use[2][0],
                   rdbg[y][x].in_use[3][0], rdbg[y][x].in_use[4][0]);
        end
        ...
        for (int x = 0; x < X_DIM; x++)
          for (int p = 0; p < NUM_PORTS; p++)
            for (int v = 0; v < VCS; v++) begin
              longint audit;
              audit = int'(rdbg[y][x].credit_free[p][v])
                    + int'(rdbg[y][x].pop_o[p][v])
                    - int'(rdbg[y][x].ack_o[p][v]);
              if (audit != 8)
                $display("DBG5 R%0d,%0d o%0d v%0d cf=%0d pops=%0d acks=%0d audit=%0d !",
                         x, y, p, v, rdbg[y][x].credit_free[p][v],
                         rdbg[y][x].pop_o[p][v], rdbg[y][x].ack_o[p][v], audit);
            end
      $display("R1 totals: injected=%0d ejected=%0d", tinj, tej);
      $fflush();
      if (tinj !== tej) begin
        $display("FAIL R1 injected != ejected (flits lost in the network)");
