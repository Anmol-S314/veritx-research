...
module noc_router #(
  parameter int VCS   = 4,
  parameter int VC_BUF = 8,
  parameter int X     = 0,
  parameter int Y     = 0,
  parameter int X_DIM = 4,
  parameter int Y_DIM = 4,
  parameter int HTREE_EN = 0,
  parameter int HTREE_LEVEL = 0
)(
  input  logic            clk,
  input  logic            rst_n,
  ...
  // occ must count 0..VC_BUF (a full buffer holds VC_BUF flits), so it
  // needs $clog2(VC_BUF)+1 bits; hp/tp are ring pointers (mod VC_BUF)
  localparam int PT_W = $clog2(VC_BUF) + 1;
  ...
  // ------------------------------------------------------------------
  flit_t      qbuf     [NUM_PORTS][VCS][VC_BUF];
  ...
  logic [4:0] credit_free[NUM_PORTS][VCS];   // init VC_BUF
...
`ifdef R1_MODE
  ...
  // Debug verbosity gate: +verbose enables per-cycle $display output.
  // Without +verbose, ALL debug prints are suppressed (100x sim speedup).
  logic dbg_v;
  initial dbg_v = $test$plusargs("verbose");
`endif
  ...
  // ------------------------------------------------------------------
  // H-tree in-network adder (ISCAS 2026 MoT): Tadd=1 after SA before XT - REAL
  logic adder_fired; logic [FLIT_BITS-1:0] adder_payload_sum; logic [7:0] adder_dst; logic [15:0] adder_pid; logic adder_is_head, adder_is_tail; logic [2:0] adder_vc; logic [7:0] adder_src;
  always_comb begin
    adder_fired=0; adder_payload_sum='0; adder_dst='0; adder_pid='0; adder_is_head=0; adder_is_tail=0; adder_vc='0; adder_src='0;
    if (HTREE_EN) begin
      for (int v0=0; v0<VCS; v0++) begin
        for (int v1=0; v1<VCS; v1++) begin
          if (front_valid[0][v0] && front_valid[1][v1] && front[0][v0].head && front[1][v1].head && out_port[0][v0]==2 && out_port[1][v1]==2 && front[0][v0].dst==front[1][v1].dst && front[0][v0].pid==front[1][v1].pid && sa_pop[0][v0] && sa_pop[1][v1]) begin adder_fired=1; adder_payload_sum=front[0][v0].data+front[1][v1].data; adder_dst=front[0][v0].dst; adder_pid=front[0][v0].pid; adder_is_head=1; adder_is_tail=front[0][v0].tail & front[1][v1].tail; adder_vc=out_vc[0][v0]; adder_src=front[0][v0].src; end
        end
      end
    end
  end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (int o = 0; o < NUM_PORTS; o++) xt_valid[o] <= 1'b0;
      if (HTREE_EN && adder_fired) begin
        xt_valid[2] <= 1'b1;
        xt_flit[2].data <= adder_payload_sum;
        xt_flit[2].head <= adder_is_head;
        xt_flit[2].tail <= adder_is_tail;
        xt_flit[2].dst  <= adder_dst;
        xt_flit[2].pid  <= adder_pid;
        xt_flit[2].vc   <= adder_vc;
        xt_flit[2].src  <= adder_src;
      end
      for (int o = 0; o < NUM_PORTS; o++) begin
        if (!(HTREE_EN && adder_fired && o==2)) begin
          for (int i = 0; i < NUM_PORTS; i++) begin
            for (int v = 0; v < VCS; v++) begin
              if (HTREE_EN && adder_fired && (i==0 || i==1) && out_port[i][v]==2 && front[i][v].head) begin
              end else if (sa_pop[i][v] && (out_port[i][v] == o)) begin
                xt_valid[o] <= 1'b1;
                xt_flit[o]  <= front[i][v];
...
`endif
      ...
      // saturate at [0, VC_BUF] per the BufferState invariant
      for (int o = 0; o < NUM_PORTS; o++) begin
        for (int v = 0; v < VCS; v++) begin
          ...
          if (credit_inc[o][v] && credit_dec[o][v])
            ...
            credit_free[o][v] <= credit_free[o][v] +
                                 ((credit_free[o][v] < VC_BUF[4:0]) ? 1 : 0);
...
`ifdef R1_MODE
          ...
          if (dbg_v && (X == 2 && Y == 6 || X == 3 && Y == 6) && i == 0 && v == 0) begin
            logic in_use_chg;
          ...
          if (do_write && flit_in[i].flit.head && (occ[i][v] > 0) &&
              !qbuf[i][v][(tp[i][v] - 1) % VC_BUF].tail)
            $display("INTLV t=%0d R%0d%0d i%0d v%0d oc%0d fp%0d fd%0d ft%b np%0d nd%0d nt%b",
                     tick_r, X, Y, i, v, occ[i][v],
                     qbuf[i][v][(tp[i][v] - 1) % VC_BUF].pid,
                     qbuf[i][v][(tp[i][v] - 1) % VC_BUF].dst,
                     qbuf[i][v][(tp[i][v] - 1) % VC_BUF].tail,
                     flit_in[i].flit.pid, flit_in[i].flit.dst,
                     flit_in[i].flit.tail);
          for (int o = 0; o < NUM_PORTS; o++)
          ...
          if (tick_r >= 66360 && tick_r < 66460 && X == 1 && Y == 6 &&
            ...
            if (sa_pop[2][0])
              $display("D16p t=%0d p2 popped fh%b ft%b fp%0d fd%0d n2h%b n2p%0d n2d%0d",
                       ...
                       qbuf[2][0][(hp[2][0] + 1) % VC_BUF].head,
                       qbuf[2][0][(hp[2][0] + 1) % VC_BUF].pid,
                       qbuf[2][0][(hp[2][0] + 1) % VC_BUF].dst);
          end
...
`endif
          ...
          if (do_write) begin
            ...
            tp[i][v] <= (tp[i][v] + 1) % VC_BUF;
          end
          if (do_pop) begin
            hp[i][v] <= (hp[i][v] + 1) % VC_BUF;
          ...
          case (st[i][v])
            S_IDLE: begin
              if (do_write && flit_in[i].flit.head) begin
                st[i][v] <= S_VA_REQ;
                out_port[i][v] <= xy_dor(X[7:0], Y[7:0],
                                         flit_in[i].flit.dst % X_DIM,
                                         flit_in[i].flit.dst / X_DIM);
              ...
              if (do_pop) begin
                if (front[i][v].tail) begin
                  if (occ[i][v] > 1) begin
                    ...
                    out_port[i][v] <= xy_dor(
                      X[7:0], Y[7:0],
                      qbuf[i][v][(hp[i][v] + 1) % VC_BUF].dst % X_DIM,
                      qbuf[i][v][(hp[i][v] + 1) % VC_BUF].dst / X_DIM);
                  end else begin
                    st[i][v] <= S_IDLE;
                  end
                  ...
                  if (qbuf[i][v][(hp[i][v] + 1) % VC_BUF].head) begin
                    st[i][v] <= S_VA_REQ;
                    out_port[i][v] <= xy_dor(
                      X[7:0], Y[7:0],
                      qbuf[i][v][(hp[i][v] + 1) % VC_BUF].dst % X_DIM,
