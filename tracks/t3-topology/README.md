...
### Where outputs land (by producer)

Every command writes to one documented place — nothing scatters:

| Producer | Writes to | Contents |
|---|---|---|
| `make sim` (booksim sweep) | `results/<CONFIG>/topology_sweep.json` | per-topology latency/throughput rows |
| `make timeloop` | `results/<CONFIG>/traffic_matrix.txt`, `timeloop.stats.txt` | traffic matrix + area/power stats |
...
```
results/                            ← tracked: committed evidence (CONFIG-named)
run/                                ← tracked: env.sh (PATH/ASTRASIM_BIN probe)
configs/                            ← tracked: template .cfg per topology
dse/veritx_dse/  dse/tests/         ← tracked: the CLI + its suite
dse/inputs/  dse/examples/          ← tracked: traces, models, sample requests
scripts/                            ← tracked: analysis pipeline (archive/ = frozen)
booksim-ext/                        ← tracked: matrix-traffic BookSim patch
log/  dse/runs/  .scratch/  .noc_p0/ ← RUNTIME ONLY (git-ignored): sim logs,
                                       scratch run dirs, RTL calibration scratch
```

Runtime dirs are safe to delete anytime; each is regenerated by the command that
owns it. If a README example references one, it recreates it.
...
**Every sweep row carries real measurements** (no placeholders):

| Field | Meaning |
|---|---|
| `astrasim_cycles` | max wall cycles across ranks (slowest rank gates the collective) |
| `latency_cycles` | same value — in embedded mode the collective's wall time IS its latency |
| `exposed_comm_cycles` | exposed communication of the slowest rank (frontend-reported) |
| `comm_overhead_pct` | exposed / wall × 100 |
| `plat_stats` | per-packet network measurements from the frontend's `[plat]` line: packet count, latency avg/min/p50/p95/p99/max (cycles), hop avg/min/max — exact retire-time samples, not estimates |
| `hops_avg` | convenience copy of `plat_stats.hops_avg` |

`plat_stats` is `null` when the frontend binary predates the `[plat]`
feature (rebuild with `third_party/astra-sim/build/astra_booksim2/build.sh`
to get it). `hops_avg`/energy proxies stay `null` alongside it — the
runner never fabricates numbers for missing measurements.
...
> **Operations notes (learned the hard way):**
> - The frontend is spawned with `stdin=DEVNULL` — its post-simulation
>   command loop blocks forever on an inherited interactive terminal
>   (symptom: `TimeoutExpired` *after* all `sys[i] finished` lines
>   printed). If you spawn the binary by hand for debugging, redirect
>   `</dev/null` or it will idle at the `Waiting` prompt after finishing.
> - Every such hang leaves an orphan `AstraSim_BookSim2` burning 100% CPU:
>   `pgrep -af AstraSim_BookSim2` and `kill` leftovers before re-running.
> - After ANY rebuild, verify the hardlink pair
>   (`third_party/astra-sim/.../bin/AstraSim_BookSim2` and the
>   `third_party/llmservingsim/...` copy) still shares an inode + md5 —
>   see `HANDOFF-2026-09-08-astra-integration.md` §3.
...
### 2026-09-10 — Per-packet latency is real now (`[plat]`), stdin hang fixed, include shadow killed

- **`[plat]` line** — the frontend now reports exact per-packet latency
  percentiles + hop counts at end of every round
  (`packets avg min p50 p95 p99 max hops_avg hops_min hops_max`),
  measured at flit retirement (`EmbedTM::PlatStats`). Wired into both
  the `t3 astrasim` sweep rows (`plat_stats`) and `veritx evaluate astra`
  result JSON. Previously embedded mode discarded every sample (flits
  were built with `record=false` and `_sim_state` was uninitialized —
  the retire gate depended on garbage memory).
- **stdin hang** — `run_astrasim.py` spawned the frontend with inherited
  stdin; the binary finished the sim then blocked forever at its command
  loop → 1800s `TimeoutExpired` with a perfectly good result sitting in
  stdout. Fixed with `stdin=subprocess.DEVNULL` + regression test.
- **Include-order shadow** — the frontend CMakeLists hardcoded the
  *nested* booksim fork's include dirs, which shadowed the canonical
  `third_party/booksim2` headers the build actually links. BookSim2
  headers now resolve from `BOOKSIM2_SRC_DIR` (the same tree whose
  objects are linked) — adding a member to `EmbedTM` no longer breaks
  the build with "not a member of" errors.
