...
3. compares the regenerated bytes with the committed fixtures
   **byte-for-byte**, and reports any difference. No producer-volatile
   ...
   reported engine defect.

### Reported engine defect (gate 10)

The committed `evaluated-design.json` and `optimization-study.json` are **not
byte-reproducible** on a fresh run. The persisted evidence bytes
(`backend/evidence.py` + `backend/booksim.py`) embed two non-semantic runtime
fields — `backend_dir` (the absolute run directory, including the OS-atomic
`mkdtemp` slot) and `wall_time_s` (measured wall time). Because
`evaluation.evidence.raw_evidence_digest` is the sha256 of those bytes, every
derived identity (`performance_result_id`, the requirement-report identity,
`optimization_result_id`) moves on a fresh run.

The provisioned gate FAILS on these two fixtures and prints:

```
ENGINE DEFECT (REPORTED, NOT NORMALIZED) <fixture>: ...
```

The fix belongs in the frozen engine (exclude runtime provenance from the
evidence digest domain, or make it deterministic). The checker does not strip
the fields and does not compare normalized documents.
